...
#Nikola Rasevic
#note: 3 hashmarks (###) represent the answer to the question.
...
###11 variables and 234 observations
view(mpg)
dim(mpg)
...
ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, color = factor(manufacturer)))
...
ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, color = factor(manufacturer),
                                              shape = factor(drv), size = factor(cyl)))

# Question 5. Modify the code to add a suitable title of your choice to your plot
plot <- ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, color = factor(manufacturer),
                                              shape = factor(drv), size = factor(cyl)))

plot+ggtitle("Highway Fuel Efficiency vs. Engine Size")+theme(plot.title = element_text(hjust = 0.5))
